-- dvě generace: node potomek parent a parent potomek grand_parent, kořen je z grand_parent, parent pak obsahuje všechny rodiče jen do kořene podstromu
-- a depth je hloubka jen v podstromu 
...
FROM
    (SELECT 
        node.uid, 
        (COUNT(parent.uid) - 1) AS depth, node.left_node, node.right_node, node.parent_uid 
    FROM
        menu_nested_set AS node
			CROSS JOIN 
		menu_nested_set AS parent ON node.left_node BETWEEN parent.left_node AND parent.right_node
			CROSS JOIN 
        menu_nested_set AS grand_parent ON parent.left_node BETWEEN grand_parent.left_node AND grand_parent.right_node
    WHERE grand_parent.uid = '5d4be52f596c7' 
    GROUP BY node.uid
	HAVING depth BETWEEN 0 AND 5
    ) AS nested_set
        INNER JOIN
