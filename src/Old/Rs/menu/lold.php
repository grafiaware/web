...
<?php
IF ($lan1) {echo '<a href="index.php?lang=lan1'; 
...
IF ($lan2) {echo '<a href="index.php?lang=lan2';
        IF ($list == 'stranky') {echo '&list=rl';} else {echo '&list='.$list;} /* rl */
        echo '&stranka=' . $stranka . '"' ;
        IF ($lang=='lan2') {echo 'class=polozkaon>';} else {echo 'class=polozka>';}
        echo $lan2[1] . "</a>";
}
IF ($lan3) {echo '<a href="index.php?lang=lan3';
        IF ($list == 'stranky') {echo '&list=rl';} else {echo '&list='.$list;}  /* rl */
        echo '&stranka=' . $stranka . '"';
...
IF ($stranka{0} == $menu_l) {
    echo '<a href="index.php?lang='.$lang.'&stranka=' . $menu_l . '" class=polozkaon>Stránky v hlavním menu</a>';}
ELSE {
    echo '<a href="index.php?lang='.$lang.'&stranka=' . $menu_l . '" class=polozka>Stránky v hlavním menu</a>';}
}
if ($zobraz_prvek['horni_menu'] ) { 
IF ($stranka{0} == $menu_h) {
    echo '<a href="index.php?lang='.$lang.'&stranka=' . $menu_h . '" class=polozkaon>Stránky v horním menu</a>';}
ELSE {
    echo '<a href="index.php?lang='.$lang.'&stranka=' . $menu_h . '" class=polozka>Stránky v horním menu</a>'; }
}
if ($zobraz_prvek['vodo_menu'] ) {
IF ($stranka{0} == $menu_s) {
    echo '<a href="index.php?lang='.$lang.'&stranka=' . $menu_s . '" class=polozkaon>Stránky ve vodorovném menu</a>'; }
ELSE {
    echo '<a href="index.php?lang='.$lang.'&stranka=' . $menu_s . '" class=polozka>Stránky ve vodorovném menu</a>';}
}    

if ($zobraz_prvek['leve_menu'] ) {
IF ($stranka{0} == $menu_l ) {
      echo '<a href="index.php?lang='.$lang.'&stranka=' . $menu_l . '" class=polozkaon>Stránky v levém menu</a>';}
ELSE {
      echo '<a href="index.php?lang='.$lang.'&stranka=' . $menu_l . '" class=polozka>Stránky v levém menu</a>';}
}
if ($zobraz_prvek['prave_menu'] ) {
IF ($stranka{0} == $menu_p ) {
      echo '<a href="index.php?lang='.$lang.'&stranka=' . $menu_p .'" class=polozkaon>Stránky v pravém menu</a>';}
ELSE {
      echo '<a href="index.php?lang='.$lang.'&stranka='. $menu_p . '" class=polozka>Stránky v pravém menu</a>';}
}
...
$i = 0;
$data = array (0=> array());
$vysledek = MySQL_Query("SELECT list,$db_nazev,$db_aktiv,$db_aktivstart,$db_aktivstop FROM stranky
                        WHERE (left(list,1)='$db_stranka') AND
                              (char_length(list)='$rozsah_urovne') ORDER BY `poradi` ASC");
WHILE ($zaznam = MySQL_Fetch_Array($vysledek)) {
        $data[0][$i]= array($zaznam['list'],$zaznam[$db_nazev],$zaznam[$db_aktiv],$zaznam[$db_aktivstart],$zaznam[$db_aktivstop]);
        $i++;
}
...
WHILE ($i <= $uroven) {
        ...
        $data [$i]=array();
        $j=0;
        $vysledek = MySQL_Query("SELECT list,$db_nazev,$db_aktiv,$db_aktivstart,$db_aktivstop FROM stranky WHERE (left(list,$delkacastlist)='$castlist') AND (char_length(list)='$delkalist') ORDER BY `poradi` ASC");
        WHILE ($zaznam = MySQL_Fetch_Array($vysledek)) {
              $data [$i][$j]= array($zaznam['list'],$zaznam[$db_nazev],$zaznam[$db_aktiv],$zaznam[$db_aktivstart],$zaznam[$db_aktivstop]);
              $j++;
        }
        $i++;
}
...
foreach ($data[$i] as $polozka) {
       IF (@$sess_prava[$polozka[0]]) {
            ...
            IF ($stranka == $polozka[0]) {
                  echo '<a href="index.php?list=rl&stranka='.$polozka[0].'&language='.$lang.'" '.$defurovne[$i].'on >'.$navest.$polozka[1].'</a>';
            }
            ELSE {echo '<a href="index.php?list=rl&stranka='.$polozka[0].'&language='.$lang.'" '.$defurovne[$i].' >'.$navest.$polozka[1].'</a>';}
       }
...
<DIV class=middmenu>
<?php
//Dotaz a zobrazeni odkazu na uverejnene stranky
...
//Dotaz a zobrazeni odkazu na neuverejnene stranky
