...
function createDatabaseContent(Psr\Container\ContainerInterface $mwContainer, $list, $lang, $edit, $klic) {
    $article = (new Model\Repository\MenuItemAggregateRepo($mwContainer->get(\Model\Dao\PaperContentDao::class)))->get($lang, $list);
    if($article) {
        if ($edit) {
            include Web\Middleware\Web\AppContext::getScriptsDirectory().'templates/layout/body/telo/tiny_config.php';    // tinymce
...
//        konstStaffer_dotazovaci_formular => Middleware\Staffer\AppContext::getScriptsDirectory()."staffer/webcontents/" .konstStaffer_dotazovaci_formular. ".php",
    ...
    if (array_key_exists($list, $contentSouce)) {
        if (is_callable($contentSouce[$list])) {
            ...
            $fileName = Web\Middleware\Web\AppContext::getScriptsDirectory().$contentSouce[$list];
