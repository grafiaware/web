...
class CampaignProvider implements CampaignProviderInterface {
    ...
    private function configData($campaignName) {
        switch ($campaignName) {
            case self::JEDNA:
                $data = [
                    'sourceCsvFilepath' => ConfigurationCache::mail()['filesDirectory'] . "VPV_ankety.csv",
                    'verifiedCsvFilepath' => ConfigurationCache::mail()['filesDirectory'] . "VPV_ankety_validated.csv",
