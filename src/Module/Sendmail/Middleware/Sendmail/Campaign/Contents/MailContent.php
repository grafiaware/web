...
class MailContent implements MailContentInterface {
    ...
    public function setAssembly($assemblyName): void {
        $this->assemblyName = $assemblyName;
    }


    public function getParams($mailAdresata, $jmenoAdresata): Params {
        if (!isset($this->assemblyName)) {
            throw new LogicException("Není nastaveno jméno pomocí metody setAssembly.");
        }
        ...
        switch ($this->assemblyName) {
            case CampaignProviderInterface::JEDNA:
                    $subject =  "Veletrh práce a vzdělávání - nabídka";                                        
                    $body = $this->htmlMessageFactory->create(__DIR__."/Messages/{$this->assemblyName}.php",
                                                        ['doSestavy' => ' ..TOTO JE varianta Jedna..',
