...
INSERT INTO `paper_content` ( `menu_item_id_fk`, `list`, `content`, `active`, `show_time`, `hide_time`, `editor`, `updated`)
SELECT   id AS menu_item_id_fk, stranky_innodb.list, obsah_lan1 AS content, aktiv_lan1 AS active,  aktiv_lan1start AS show_time,  aktiv_lan1stop AS hide_time, editor, zmena  AS updated FROM stranky_innodb  INNER JOIN `menu_item`  ON  (menu_item.lang_code_fk='cs' AND stranky_innodb.list=menu_item.list)
UNION ALL
SELECT   id AS menu_item_id_fk, stranky_innodb.list, obsah_lan2 AS content, aktiv_lan1 AS active,  aktiv_lan1start AS show_time,  aktiv_lan1stop AS hide_time, editor, zmena  AS updated FROM stranky_innodb  INNER JOIN `menu_item`  ON  (menu_item.lang_code_fk='en' AND stranky_innodb.list=menu_item.list)
UNION ALL
SELECT   id AS menu_item_id_fk, stranky_innodb.list, obsah_lan3 AS content, aktiv_lan1 AS active,  aktiv_lan1start AS show_time,  aktiv_lan1stop AS hide_time, editor, zmena  AS updated FROM stranky_innodb  INNER JOIN `menu_item`  ON  (menu_item.lang_code_fk='de' AND stranky_innodb.list=menu_item.list)
ORDER BY list ASC;